...
object BrowserDriver {
  ...
  // Lazy instantiation of the driver based on system property
  ...
  // Implicit driver so existing tests can just do `driver.findElement(...)`
  implicit def driver: WebDriver = instance
}
